...
namespace fgseg {
	...
	bgs(double tau, double alpha, bool selective_bkg_update, bool rgb);
	bgs(double tau, double alpha, bool selective_bkg_update, bool rgb, double a,
			double beta, double s_tau, double h_tau);
	...
	cv::Mat counter;
	cv::Mat map;
	int _counterThreshold;

	//Variable for shadow Remove
	double _a, _beta, _s_tau, _h_tau;
	bool _gaussian_update;
	cv::Mat _mu;
	cv::Mat _variance;
	cv::Mat _std;
