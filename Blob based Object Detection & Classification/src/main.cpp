...
//		// "/home/avsa/datasets/...
	...
	/*string dataset_path =
			"/home/mohammad/eclipse-workspace/Lab.0AVS2021_task_shadow/AVSA_LAB1_dataset2012lite/dataset2012lite/dataset"; //SET THIS DIRECTORY according to your download
	...
	string dataset_path =
			"/home/mohammad/eclipse-workspace/Lab.02_AVS2020_task_01/2020AVSALab3_datasets"; //SET THIS DIRECTORY according to your download
		...
		for (int s = 0; s < NumSeq; s++) {
			...
			for (;;) {
				...
				Mat element = getStructuringElement(MORPH_RECT,
						Size(2 * kernel_size + 2, 2 * kernel_size + 2),
				...
				//Threshold the image to eliminate Shadow
				...
				// STATIONARY BLOBS
				if (it == 1) {
					sfgmask = Mat::zeros(Size(fgmask.cols, fgmask.rows),
							CV_8UC1);
					fgmask_history = Mat::zeros(Size(fgmask.cols, fgmask.rows),
							CV_32FC1);
				}
				...
				removeSmallBlobs(sbloblist, sbloblistFiltered, MIN_WIDTH,
				MIN_HEIGHT);
...
//		        if (_CONSOLE_DEBUG) cout << "proc. time = " << 1000*t/t_freq << " milliseconds."<< endl;
				...
				string title =
						project_name
								+ " | Frame - FgM - Stat FgM | Blobs - Classes - Stat Classes | BlobsFil - ClassesFil - Stat ClassesFil | ("
								+ dataset_cat[c] + "/" + baseline_seq[s] + ")";

				ShowManyImages(title, 6, frame, fgmask, sfgmask,
						paintBlobImage(frame, bloblistFiltered, false),
						paintBlobImage(frame, bloblistFiltered, true),
						paintBlobImage(frame, sbloblistFiltered, true));
